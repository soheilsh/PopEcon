...
delta = 1.8375206                   # Power of Technological growth function
B0 = 0.0236504                      # Coefficient of technological growth
...
def output(filename, sheet, list1, list2, list3, list4, list5, val1, x, y, z, w):
    ...
    for indx , m in enumerate(list5, 1):
        sh.write(n + 10, indx, m)

    sh.write(n + 11, 1, val1)    
    book.close()
...
for r1 in l1:
    for r2 in l2:
        ...
        ts = 1.3 * r2                        # Time spent to raise an skilled child
        ...
        h = [0]*T                           # Ratio of skilled labor in Manufacturing to population
...
# ============================================ Initializing @ year 1840 ============================================ #
        ...
        h[0] = mt[0] / Pop[0]
        Y[0] = GDPData[0]
        Am[0] = Y[0] / ((1 - eps) * (mt[0] / lmt[0])**eps * lat[0] + (mt[0])**eps * (lmt[0])**(1 - eps))
...
#                axes.set_ylim([1.5,2.5])
        ...
        output('Results.xlsx', str(r1) + '-' + str(r2), nopt, Pop, Y, h, er2, Eval, alpha, beta, tu, ts)
