...
C_0 = 0.00                          # Minimum level of Agricultural consumption
    ...
    for t in range(tx, tx + 40):    
        if t<tx + 20:
            if typ == 1:                            #Unskilled
                ...
                W[0, t-tx] = alpha * np.log(ca[0, t] - C_0) + beta * np.log(cm[0, t]) + (1 - alpha - beta) * np.log(n[0] * np.mean(wu[tx+20:tx+60]) + n[1] * np.mean(ws[tx+20:tx+60]))
            else:                                   #Skilled
                ca[1, t] = (ws[t] * (1-gammas[0, t]) + (beta/alpha) * pa[t] * C_0)/((1+ (beta/alpha)) * pa[t])
                cm[1, t] = (beta/alpha) * pa[t] * (ca[1, t] - C_0)
                W[1, t-tx] = alpha * np.log(ca[1, t] - C_0) + beta * np.log(cm[1, t]) + (1 - alpha - beta) * np.log(n[0] * np.mean(wu[tx+20:tx+60]) + n[1] * np.mean(ws[tx+20:tx+60]))
        else:
            if typ == 1:                            #Unskilled
                ca[0, t] = (wu[t] * (1-gammau[0, t]) + (beta/alpha) * pa[t] * C_0)/((1+ (beta/alpha)) * pa[t])
                cm[0, t] = (beta/alpha) * pa[t] * (ca[0, t] - C_0)
                W[0, t-tx] = alpha * np.log(ca[0, t] - C_0) + beta * np.log(cm[0, t])
            else:                                   #Skilled
                ca[1, t] = (ws[t] * (1-gammas[0, t]) + (beta/alpha) * pa[t] * C_0)/((1+ (beta/alpha)) * pa[t])
                cm[1, t] = (beta/alpha) * pa[t] * (ca[1, t] - C_0)
                W[1, t-tx] = alpha * np.log(ca[1, t] - C_0) + beta * np.log(cm[1, t])
        U[typ-1, tx] = U[typ-1, tx] + W[typ-1, t-tx]/(1 + ro)**(t - tx)
    return -U[typ-1, tx]
...
#    np.savetxt("res.csv", nopt, delimiter=",")